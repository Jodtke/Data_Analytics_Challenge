...
setwd("/Users/ejahnke/Desktop/Recommendations_24062021")
getwd()
...
items6 <- read.csv("items6.csv", encoding = "UTF-8")

# Datensatz mit Transaktionsdaten
transactions <- as_tibble(read.csv("transactions.csv", header = T, sep = "|", quote = "", row.names = NULL, stringsAsFactors = F))

# Crawler-Daten mit Klappentexten
FCD_tibble <- as_tibble(read.csv("KlappentexteUndTitel.csv", encoding = "UTF-8"))

# Evaluations-Daten
evaluation <- read.csv("evaluation.csv")

### Datensätze transformieren und Ergebnisdatensätze erstellen
...
# Ähnlichkeit zwischen den Autoren berechnet.
# Dabei wird Berücksichtigt, wie viele Bücher die Autoren im jeweiligen
# Main Topic und Sub Topic geschrieben haben

# Imporiteren der AuthMat_subtopics aus anderem Skript
AuthMat <- read.csv("AuthMat_subtopics.csv", check.names = FALSE)
...
itemMat <- read.csv("itemMat_subtopics.csv", check.names = FALSE)
...
# Zusammenfügen der Transaktions- und Item-Daten, welche als Ausgangspunkt
# zur Gewinnung weiterer Features dienen
joined_oR <- left_join(transactions, items6, by = "itemID")

# Erstellung von dup.ses, um im späteren Verlauf eine Art ARM anwenden zu können
dup.ses <- joined_oR[joined_oR$sessionID %in%
                       joined_oR$sessionID[duplicated(joined_oR$sessionID)],]

# Erstellung der Variable inDupSes, welche darüber informiert, ob sich ein Item
...
# Erstellung der Variable inTransactions, welche darüber informiert, ob sich ein 
...
# Erstellung der Variable clicked, welche darüber informiert, ob ein Item 
...
# Erstellung der Variable basket, welche darüber informiert, ob ein Item 
...
# Erstellung der Variable ordered, welche darüber informiert, ob ein Item 
...
# Da diverse Bücher mit dem identischen Titel vorliegen, die Item-Daten mit den
# Crawler-Daten jedoch über die Titel gejoint werden, müssen zunächst alle
...
# Erstelle tibble totalInfo, welches folgende Informationen enthält:
# - itemID
# - titel
# - autor
# - publisher
...
# Verknüpfung des Evaulation-Tibbles mit totalInfo um sämtliche Informationen
# über die Ziel-Items zu erhalten
...
# müsste der Code mehrmals kopiert und eingefügt werden.Durch die Definition 
...
# um so Buch/Item-Empfehlungen aussprechen zu können. Diese werden dann im
...
# ErgebnisMMC
# 
# optWeights <- colMeans(ErgebnisMMC[, 3:5])
# optWeights
# sum(optWeights)

# Teil, zur Erstellung der 200 Items pro Person

forItems <- 1000 # damit das mit der Schleife funktioniert

Recommendations <- matrix(NA, ncol = 6, nrow = forItems)
Recommenations_names <- c("activeItem", "Rec1", "Rec2", "Rec3", "Rec4", "Rec5")
colnames(Recommendations) <- Recommenations_names

itemIDs <- as.character(activeAuthorAndTitel$itemID[1:forItems])
...
meinBereich <- 945:1000

tic("Start Recommendations")

for(idx in meinBereich){
  
  activeItemID <- itemIDs[idx]
...
# write-Befehl um Ergebnisse als CSV zu speichern.
...
# # Für John
# 
# first100 <- as_tibble(readRecommendations[1:100, ])
# names(first100) <- c("id", "rec_1", "rec_2", "rec_3", "rec_4", "rec_5")
